...
class DepthPreparer:
	...
	def __init__(self, fileManager, workers = None):
		...
		self.fileManager.downloadProjectData(dtype= 'Depth')
		# pdb.set_trace()
		self.createLogFile()
	...
	# def createSmoothedArray(self, goodDataCutoff = 0.65, minimumGoodData = 0.9, std_cutoff = 0.25, max_depth = 6, max_height = 10):
	...
	# 	#originalData = depthData.copy()
	# 	#nan_values_raw = np.isnan(depthData).sum(axis = (1,2))
	...
	# 	daytime_data = depth_dt[(depth_dt.DaytimeData == True) & (~depth_dt.Trial.str.contains('Reset'))].groupby(['Trial','RelativeDay']).agg(first_index = ('Index','first'), last_index = ('Index','last'))
	# 	# Loop through each day and interpolate missing data, setting night time data to average of first and last frame
	# 	night_start = 0
	# 	for (trial,day), (start_index,stop_index) in daytime_data.iterrows():
			
	# 		dailyData = depthData[start_index:stop_index+1] # Create view of numpy array just creating a single day during the daytime
	...
	# 		ImageDraw.Draw(img).polygon(depth_crop_points, outline=1, fill=1)
	# 		manual_crop_mask = np.array(img)
	# 		dailyData[:,manual_crop_mask == 0] = np.nan
	# 		lastDepthRGB = Image.open(self.fileManager.localProjectDir + self.fileManager.lp.frames[6269].pic_file)
	# 		goodDataAll = np.count_nonzero(~np.isnan(dailyData), axis = 0)/dailyData.shape[0] # Calculate the fraction of good data points per pixel
	...
	# 	# Save interpolated data
	# 	#nan_values_pre_mask = np.isnan(depthData).sum(axis = (1,2))
	# 	# Read in manual crop and mask out data outside of crop
	# 	with open(self.fileManager.localDepthCropFile) as f:
	# 		for line in f:
	# 			depth_crop_points = eval(line.rstrip())
	...
	# 	ImageDraw.Draw(img).polygon(depth_crop_points, outline=1, fill=1)
	# 	manual_crop_mask = np.array(img)
	# 	depthData[:,manual_crop_mask == 0] = np.nan
	# 	#nan_values_smooth = np.isnan(depthData).sum(axis = (1,2))
	...
	# 			self.lp.frames[start_index].time, self.lp.frames[last_index].time, masked = False), vmin=-3, vmax=3)
	...
	# 		topAx2_ax = topAx2.imshow(self.da_obj.returnHeightChange(self.lp.frames[reset_index].time, self.lp.frames[last_index].time, masked = False), vmin = -3, vmax = 3)
	...
	# 				current_axs[1].imshow(self.da_obj.returnHeightChange(self.lp.frames[day_start-1].time, self.lp.frames[day_stop].time, masked=False), vmin=-v, vmax=v)
	# 				current_axs[2].imshow(self.da_obj.returnHeightChange(self.lp.frames[day_start-1].time, self.lp.frames[day_stop].time, masked=True), vmin=-v, vmax=v)
	# 			elif j==num_days - 1:
	...
	# 				bowerVolume = self.da_obj.returnVolumeSummary(self.lp.frames[day_start].time,self.lp.frames[day_stop+1].time).depthBowerVolume
	# 				current_axs[0].set_title(str(day) + ': ' + str(int(bowerVolume)))
	# 				current_axs[1].imshow(self.da_obj.returnHeightChange(self.lp.frames[day_start].time, self.lp.frames[day_stop+1].time, masked=False), vmin=-v, vmax=v)
	# 				current_axs[2].imshow(self.da_obj.returnHeightChange(self.lp.frames[day_start].time, self.lp.frames[day_stop+1].time, masked=True, ), vmin=-v, vmax=v)
	# 			else:
	# 				current_axs[0].imshow(self.da_obj.returnHeightChange(self.lp.frames[day_info.iloc[-1].day_start].time, self.lp.frames[day_stop + 1].time, cropped=True), vmin=-v, vmax=v)
	...
	# 				current_axs[0].set_title(str(day) + ': ' + str(int(bowerVolume)))
	# 				current_axs[1].imshow(self.da_obj.returnHeightChange(self.lp.frames[day_start-1].time, self.lp.frames[day_stop+1].time, masked=False), vmin=-v, vmax=v)
	# 				current_axs[2].imshow(self.da_obj.returnHeightChange(self.lp.frames[day_start-1].time, self.lp.frames[day_stop+1].time, masked=True), vmin=-v, vmax=v)
	...
	def createDepthFigures(self, hourlyDelta=2):
		...
		for i in range(self.lp.numDays):
			...
			bower_data = self.da_obj.returnVolumeSummary(start,stop)
			current_axs[0].set_title('Day %i' % (i + 1) + f': {bower_data.thresholdPitVolume:.2f},{bower_data.thresholdCastleVolume:.2f}',
		...
		figHourly = plt.figure(figsize=(11, 8.5))
		gridHourly = plt.GridSpec(self.lp.numDays, int(11 / hourlyDelta) + 2, wspace=0.5, hspace=0.05)
		bounding_ax = figHourly.add_subplot(gridHourly[:, :])
		bounding_ax.xaxis.set_visible(False)
		bounding_ax.set_ylabel('Day')
		bounding_ax.set_ylim(self.lp.numDays + 0.5, 0.5)
		...
		for i in range(0, self.lp.numDays):
			...
			for j in range(int(24 / hourlyDelta)):
				...
				if i == 0:
					current_ax.set_title(str(j * hourlyDelta) + '-' + str((j + 1) * hourlyDelta))
				current_j += 1
				# current_ax.set_title(':'+str(bower_data.thresholdPitVolume)+','+str(bower_data.thresholdCastleVolume),rotation = 45)
				current_ax.text(1.05, 0.5, f': {bower_data.thresholdPitVolume:.2f}, {bower_data.thresholdCastleVolume:.2f}',
                transform=current_ax.transAxes, ha='left', va='center', fontsize=5,rotation = 90)
				# pdb.set_trace()
			current_ax = figHourly.add_subplot(gridHourly[i, -2])
			current_ax.imshow(self.da_obj.returnBowerLocations(stop - datetime.timedelta(hours=24), stop, cropped=True),
							  vmin=-v, vmax=v)
			...
			if i == 0:
				current_ax.set_title('Daily\nChange')
			figHourly.tight_layout(pad=2.0) 
