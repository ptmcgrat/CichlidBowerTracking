	...
	def labelVideos(self, initials, Nfilter):
		...
		while index < len(clips): # We use a while loop so we can reannotate a clip if a mistake is made
			...
			if clip_name in labeled_dt.ClipName.values:
				...
				labeled_dt.loc[len(labeled_dt)] = [self.fileManager.analysisID, clip_name, chr(info), self.initials, str(datetime.datetime.now()), True, self.fileManager.projectID, None] # Create new annotation
