	...
	def createSummaryFigures(self, v=2.0):
		...
		for i,trial in enumerate(self.lp.trials):
			...
			for j, (first_frame,last_frame) in enumerate(trial.days):
				...
				x3,y3 = self.cl_obj.returnDepthCoordinates(first_frame.time, last_frame.time, 'b')
				y3 = self.da_obj.height - y3
				axes[2,j].scatter(x3,y3,s = 0.05, color = 'green')
				...
				x3,y3 = self.cl_obj.returnDepthCoordinates(first_frame.time, last_frame.time, 'b', sex = 'Male')
				y3 = self.da_obj.height - y3
				...
				y = self.da_obj.height - y
				axes[7,j].scatter(x,y,s = 0.05, color = 'black')
